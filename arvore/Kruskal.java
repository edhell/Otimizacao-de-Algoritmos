...
public class Kruskal {
	private final int MAX_NODES = 21;			// Maximo de NODES
	...
	// Roda o algoritmo informando um arquivo:
	...
	// Roda o algoritmo informando uma lista de itens: 
    public void run2(List<ArvoreItem> itens){
        System.out.println("Rodando [Kruskal] 2 - Informando itens");
        
        Kruskal k = new Kruskal();
        k.lerDados2(itens);			// Pega todas as informações
        k.executarKruskal();		// Executa o algoritmo Kruskal
        k.imprimirCaminho();		// Imprime o resultado
    }
  
	// Faz a leitura e extração dos itens do arquivo:
	...
	// Faz a leitura e extração dos itens informados
    private void lerDados2(List<ArvoreItem> itens) {
        try {
            for (int i = 0; i < itens.size(); i++){
                int from = itens.get(i).from;	// Inicio
                int to   = itens.get(i).to;		// Destino
                int cost = itens.get(i).cost;	// Custo
                
                allEdges.add(new Edge(from, to, cost));  // Atualizar fila de prioridade
                if (nodes[from] == null) {
                    // Criando um conjunto de componentes de conexão para este nó
                    nodes[from] = new HashSet(2*MAX_NODES);
                    nodes[from].add(new Integer(from));
                }
	...
	// Verifica se nó estão em conjuntos diferentes:
	...
	// Classe interna para representar arestas e pontos finais
